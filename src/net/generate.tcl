...
public:
  INO_${name}(NetworkConnection*);
protected:
  virtual void construct() throw();
  virtual void update() throw();

private:
  static $name* decodeConstruct(const std::vector<byte>&) throw();
  static bool decodeUpdate(const std::vector<byte>&, $name*) throw();
};
...
"
  ...
  puts $cout \
"INO_${name}::INO_${name}(NetworkConnection* cxn)
: ImportedGameObject($byteOffset, cxn)
{ }
...
void INO_${name}::update() throw() {
  if(decodeUpdate(state, object, true))
    destroy();
}

$name* INO_${name}::decodeConstruct(const std::vector<byte>& DATA) throw() {
  [cxxj declaration]
  [cxxj decode]
  $name* X;
  $typeConstructor
...
void INO_${name}::decodeUpdate(const std::vector<byte>& DATA, $name* X)
throw () {
  bool DESTROY = false;
  [cxxj declaration]
  [cxxj decode update]
...
bool ENO_${name}::shouldUpdate() const throw() {
  ...
  [cxxj xalias declaration unalias yalias declaration unalias]
  #define X local
  [cxxj xalias]
  [cxxj extract]
  [cxxj unalias]
  ...
  float l_dist = cxn->distanceOf(l_local);
...
proc aliases {name} {
  global current
  dict set current xalias "#define $name x_$name"
  dict set current yalias "#define $name y_$name"
  dict set current unalias "#undef $name"
}
...
proc float {name {parms {}} {save yes}} {
  ...
  dict set current encode "io::write_c(&[data], $name, 4);"
  dict set current decode "io::read_c(&[data], $name, 4);"
  eval-parms $parms
  incr byteOffset 4
  if {$save} save
}
...
proc str {maxlen name {parms {}} {save yes}} {
  ...
  "strncpy($name, [data], $maxlen-1); $name\[$maxlen-1]=0;"
  dict set current encode \
  "strncpy([data], $name, $maxlen);"
  eval-parms $parms
